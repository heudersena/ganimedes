...
function BR(moeda) {
    return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(moeda));
}
    ...
    <table v-else class="table-striped">
      ...
      <tbody>
        <tr v-for="(saque, i) in saques" :key="i" class="cursor-pointer">
          ...
          <td class="uppercase" :class="saque.status === 'processing' ? 'text-yellow-500 font-semibold' : ''">{{
            saque.status ===
            'processing' ? 'EM PROCESSO' : saque.status }}</td>
          <td>{{ saque.profile.email }}</td>
          <td>{{ saque.profile.phone }}</td>
          <td>{{ saque.profile.keyPix }}</td>
          <td>
            <RouterLink :to="'saque-editar/' + saque.id + '/' + saque.profile.keycloak_id" class="cursor-pointer">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
